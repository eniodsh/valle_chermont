 ...
 */
	public function error($field, $text = null, $options = array()) {
		$defaults = array('wrap' => 'span', 'class' => 'help-inline', 'escape' => true);
 ...
 */
	public function input($fieldName, $options = array()) {
		...
		$out = array_merge(
			array('before' => null, 'label' => null, 'between' => null, 'input' => null, 'after' => null, 'error' => null, 'controlBegin' => $div ? '<div class="controls">' : '', 'controlEnd' => $div ? '</div>': ''),
		...
		switch ($type) {
			...
			case 'checkbox':
				...
				$format = $format ? $format : array('controlBegin', 'before', 'input', 'between', 'label', 'error', 'after', 'controlEnd');
			break;
			case 'radio':
			...
			case 'date':
				$options['value'] = $selected;
				$input = $this->data($fieldName, $options);
			break;
			case 'datetime':
				$options['value'] = $selected;
				$input = $this->data($fieldName, $options);
		...
		$format = $format ? $format : array('before', 'label', 'between', 'controlBegin' ,'input', 'error', 'after', 'controlEnd');
		$output = '';
 ...
 */
	protected function _inputLabel($fieldName, $label, $options) {
		...
		if ($options['type'] !== 'checkbox') {
			$labelAttributes = $this->addClass($labelAttributes,'control-label');
		}
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		foreach ($options as $optValue => $optTitle) {
			...
			$outItem = $this->Html->useTag('radio', $attributes['name'], $tagName,	array_diff_key($allOptions, array('name' => '', 'type' => '', 'id' => '')), '');		
			if ($label) {
				$outItem = $this->Html->useTag('label', $tagName, array('class' => 'radio'), $outItem . ' ' . $optTitle);
			}
			$outItem .= $between;
			$out[] = $outItem;
...
/**
 * Cria um campo de data em português.
 *
 *
 * @param string $fieldName Name of a field, in the form "Modelname.fieldname"
 * @param array $options Array of HTML attributes, and special options above.
 ...
 */
	public function data($fieldName, $options = array()) {
		...
		$options = $this->addClass($options, 'data');
		$value = null;

		if (array_key_exists('value', $options)) {
			if(!preg_match("^\d{1,2}/\d{1,2}/\d{2,4}^", $options['value'])) {
				$options['value'] = d($options['value']);
			}
		}
