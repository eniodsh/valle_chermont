...
var AdminNoticiasForm = new function() {
    ...
    this.execute = function(form) {
        ...
        AdminNoticiasForm.form = form;

        // adiciona e executa as regras
        AdminNoticiasForm.addFieldValidade();
    };
    ...
    this.addFieldValidade = function() {
        ...
        var fieldValidateMarkup = '<div class="control-group">' +
            ...
            '<input type="checkbox" id="validade" name="data[Noticia][indeterminado]">Intederminado' +
            '</label>' +
            '</div>' +
            '</div>';

        // procura o container de campos data
        var validade = $(AdminNoticiasForm.form).find(".validade");

        // adiciona o campo validade
        validade.before(fieldValidateMarkup);

        var fieldIndeterminado = $(AdminNoticiasForm.form).find("#validade");
        ...
        if ($("[name='data[Noticia][data_inicio]']").val() && $("[name='data[Noticia][data_fim]']").val()) {
